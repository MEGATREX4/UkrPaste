  ...
  return (
    <>
      ...
      <main className="flex-grow flex flex-col items-center p-6 max-w-xl mx-auto w-full">
        ...
        <form onSubmit={handleSubmit} className="bg-gray-800 p-6 rounded-md w-full shadow-md flex flex-col gap-4">
          <div>
            <label htmlFor="author" className="block mb-2">Ваш нікнейм або ім'я:</label>
            <input id="author" value={author} onChange={e => setAuthor(e.target.value)} className="w-full p-3 rounded bg-gray-700 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-pink-600" />
          </div>
          <div>
            <label htmlFor="content" className="block mb-2">Текст пасти:</label>
            <textarea id="content" rows="6" required value={text} onChange={e => setText(e.target.value)} className="w-full p-3 rounded bg-gray-700 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-pink-600"></textarea>
          </div>
